...
if len(argv) != 2:
	...
	if TestOn == False:	
		...
		if TestCase[caseID][0] in {1, 2, 6, 10, 11, 12, 16, 17, 18}:
			data = server.receiveData()
			if data == False: print 'No data is received.'
			else: print '\nReceived Data: \n{}\n'.format(data)
			print '\n---------------------END (Server)-----------------------'
		elif TestCase[caseID][0] == 3:
			server.rudp.pktId = 0
			...
			print '---------------------END (Server)-----------------------\n'
		elif TestCase[caseID][0] == 4 or TestCase[caseID][0] == 5:
